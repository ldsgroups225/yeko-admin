...
export function formatDate(
  date: DateInput,
  format: string = DATE_FORMAT_SHORT,
  locale: "fr" | "en" = "fr",
): string {
  return formatDateWithOptions(date, format, {
...
export function getAge(birthday: string): number {
  ...
  if (!isValid(birthDate)) {
    throw new Error(
      "Invalid birthday format. Please provide a valid ISO string date.",
    );
  }
...
export function formatTimePassed(
  date: Date | number,
  calcFromMidnight: boolean = false,
): string {
  const now = new Date();
...
export function calculateEventPosition(startTime: string): number {
  ...
  return (
    HEADER_HEIGHT +
    hoursPassed * CELL_HEIGHT +
    Math.floor(hoursPassed) * BORDER_WIDTH
  );
}
