...
import type { OAuthError, OAuthOptions, OAuthState } from "@/types";
...
export function useGoogleAuth() {
  ...
  const googleSignIn = useCallback(
    async (options?: Partial<OAuthOptions>) => {
      try {
  ...
  const linkGoogle = useCallback(
    async (userId: string) => {
      try {
        ...
        if (!result.success) {
          handleError(
            result.error || "Erreur lors de la liaison du compte Google",
          );
          return false;
        }
        ...
        handleError(
          error instanceof Error
            ? error.message
            : "Erreur inattendue lors de la liaison",
        );
        return false;
      }
    },
   ...
   */
  const checkGoogleLinked = useCallback(
...
export function useGoogleAuthSimple() {
  const { googleSignIn, googleSignUp, isLoading, error, clearError } =
    useGoogleAuth();
